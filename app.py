...
def get_all_proverbs():
    conn = sqlite3.connect('proverbs (1) (2).db')
    cursor = conn.cursor()
...
def index():
    ...
    if request.method == 'POST':
        ...
        if keyword:
            ...
            standardized_query = keyword.strip()
            exact_results = [
                row for row in all_rows
                if row[0].strip() == standardized_query or (row[1] and row[1].lower().strip() == standardized_query.lower())
            ]


            if exact_results:
                results = exact_results[:1]
            else:
                if not selected:
                    if is_romanized:
